#let note-info-check(config: (:), notes-info-after-sortd: ()) = {
  ...
  for note-info in notes-info-after-sortd {
    assert(note-info.start-row > prev-note-end, message: "note start-row should be greater than previous note end-row")
...
#let get-body(note-info) = {
  if note-info.bracket == "curly" {
    if note-info.side == "right" {
      return box(height: 100%, inset: (left: 0pt), layout(size => {
        math.lr("}", size: size.height)
      }))
    } else if note-info.side == "left" {
      return box(height: 100%, inset: (right: 0pt), layout(size => {
        math.lr("{", size: size.height)
      }))
    }
  }
...
#let note-config-to-cells(config: (:), notes-info: ()) = {
  ...
  for note-info in notes-info-after-sortd {
    ...
    cells.push(
      grid.cell(fill: note-info.fill, align: note-info.position, inset: 2pt, y: note-info.start-row, rowspan: rowspan)[
...
#let notes-to-grid(config: (:), notes-info: ()) = {
  ...
  return grid(fill: config.table-fill, align: config.table-align, columns: (1fr, 0pt), rows: config.rows-size, ..{
    for cell in cells {
      (cell,)
    }
  })
