...
#let note(
  ...
  assert(
    type(side) == str and (side == "right" or side == "left"),
    message: "Usage: side: right | left",
  )

  assert(
    type(start-row) == int and start-row > 0,
    message: "start-row should be greater than 0",
  )
  ...
  assert(
    ...
    message: "end-row should be greater than start-row",
  )
